...
<div id="infoWindowUI" style="display:none;">
    ...
    <div class="info-window-flex">
      <StatusBox :number="infoData.totalOrders" title="总订单" direction="row" clickable="clickable" @click="toggleModal('20,30,40,50,60,90', infoData.totalOrders)" :selected="(showModalTable && (orderStatusCode === '20,30,40,50,60,90')) ? 'selected' : ''" />
      <StatusBox :number="infoData.currentOrders" title="已完成" direction="row" clickable="clickable" @click="toggleModal('50,60', infoData.currentOrders)" :selected="(showModalTable && (orderStatusCode === '50,60')) ? 'selected' : ''" />
      <StatusBox :number="infoData.toBePayCount" title="待支付" direction="row" clickable="clickable" @click="toggleModal('40', infoData.toBePayCount)" :selected="(showModalTable && (orderStatusCode === '40')) ? 'selected' : ''" />
      <StatusBox :number="infoData.cancleCount" title="已取消" direction="row" clickable="clickable" @click="toggleModal('90', infoData.cancleCount)" :selected="(showModalTable && (orderStatusCode === '90')) ? 'selected' : ''" />
      <StatusBox :number="infoData.inServiceCount" title="服务中" direction="row" clickable="clickable" @click="toggleModal('30', infoData.inServiceCount)" :selected="(showModalTable && (orderStatusCode === '30')) ? 'selected' : ''" />
      <StatusBox :number="infoData.todoOrders" title="待服务" direction="row" clickable="clickable" @click="toggleModal('20', infoData.todoOrders)" :selected="(showModalTable && (orderStatusCode === '20')) ? 'selected' : ''" />
    </div>
    <mu-divider />    
    ...
    <transition name="order-table-slide">
      <ModalTable v-if="showModalTable" class="order-table" height="auto" :change="getList" />
    </transition>
    </div>
  </div>
</template>
...
import ModalTable from '../units/modalTable'
...
#infoWindowUI .license-icon {
  width: 24px;
  height: 24px;
  background-image: url(../../../static/license.png)
}
#infoWindowUI .service-icon {
  width: 24px;
  height: 24px;
...
#infoWindowUI .order-table-slide-enter-active {
  transition: all .3s ease;
}
#infoWindowUI .order-table-slide-leave-active {
  transition: all .8s cubic-bezier(1.0, 0.5, 0.8, 1.0);
}
#infoWindowUI .order-table-slide-enter, .order-table-slide-leave-active {
